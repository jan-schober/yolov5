...
augmented_list = sorted(glob.glob('/home/schober/carla/for_yolov5/images/carla_segmented_01/*.png'))
gt_flow_list = sorted(glob.glob('output_time_ren_seg/*gt.png'))
ag_flow_list= sorted(glob.glob('output_time_ren_seg/*ag.png'))
diff_flow_list= sorted(glob.glob('output_time_ren_seg/*diff.png'))
...
video_name = 'time_consistency_ren_seg.avi'
...
first_list = [segmented_list[0],segmented_list[0], augmented_list[0], gt_flow_list[0],diff_flow_list[0], ag_flow_list[0]]
...
tof_arr = np.load('tof_arr_ren_seg.npy')
...
for segmented_path, augmented_path, gt_flow_path, ag_flow_path, diff_flow_path, tofl in zip(segmented_list, augmented_list, gt_flow_list, ag_flow_list, diff_flow_list, tof_arr):
    ...
    grid = image_grid([segmented_img, black_image, augmented_img, gt_flow_img, diff_flow_img,ag_flow_img], rows=2, cols=3)
